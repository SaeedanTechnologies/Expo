...
const AddParticipant = () => {
  ...
  const [descriptions, setDescriptions] = useState(
    Array(uploadFields.length).fill("")
  );
  ...
  useEffect(() => {
    ...
    if (fileData) {
      ...
      const fileLabels = fileData.map((file) => file.label);
      setDescriptions(fileLabels);
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const extracted_array = responseData.data.map((item) => ({
        id: item.id,
        type: item.type,
      }));
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Note: You have an option to mark yes or no. If you click yes, that
          field will not be seen by the judge.
        </Typography>
        <Box
          sx={{
            display: "flex",
        ...
        >
          ...
          >
            ...
            >
              Drag & Drop field from left to right <FaArrowRight size="2em" />
            </Typography>
          </Box>
          <Box
            flex={1}
