...
//   //   formData.append('judge_name', names);
//   //   formData.append('link', 'https://frontend.saeedantechpvt.com/admin-login');
//   //   formData.append('email', judges.map(judge => judge.email));
  
//   //   // Append profile picture file to formData
//   //   formData.append('profile_picture', profile);
  
//   //   // Append textFields as JSON string (optional based on server expectation)
//   //   formData.append('fields', JSON.stringify(textFields.map(field => ({
//   //     name: field.value,
//   //     label: field.value,
//   //     type: field.type,
//   //     required: field.required,
...
//   //   axios.post('https://expoproject.saeedantechpvt.com/api/admin/judges', formData, {
//   //     headers: {
//   //       'Authorization': `Bearer ${token}`,
...
//     // Assuming these variables are defined in your component:
//     // contest_id, names (array of judge names), judges (array of judge objects), textFields, profile (File object), token
  
//     // Prepare form data for file upload
//     const formData = new FormData();
//     formData.append('contest_id', contest_id);
    
//     // Append judge names as an array
//     names.forEach(name => {
//       formData.append('judge_name[]', name);
//     });
  
//     // Append emails correctly
//     judges.forEach(judge => {
//       formData.append('email[]', judge.email);
//     });
  
//     // Append profile picture
//     formData.append('profile_picture', profile); 
  
//     // Append other fields as needed, e.g., phone number, link
//     formData.append('phone', ''); // Assuming phone is a required field and should be a string
//     formData.append('link', 'https://frontend.saeedantechpvt.com/admin-login');
  
//     // Convert textFields array to JSON string and append to formData
//     formData.append('fields', JSON.stringify(textFields.map(field => ({
//       name: field.name,
//       label: field.label,
//       type: field.type,
...
//       // Handle success
//       setLoading(false);
//       console.log('Judges added successfully:', response.data);
...
//       // enqueueSnackbar('Failed to add judges', { variant: 'error' });
//     });
//   };
...
const DraggableButton = () => {
  ...
  return (
    <Box>
      ...
      <Typography
        sx={{ fontSize: "15px", fontWeight: 600, marginTop: "12px" }}
      >
        Please Drag and Drop Left to right <FaArrowRight style={{ fontSize: '14px' }}/>
      </Typography>
    </Box>
  );
};
...
const DropArea = ({ onDrop, children }) => {
  ...
  return <Box ref={drop} sx={{ minHeight: "200px", padding: "1rem" }}>{children}</Box>;
};
...
const CreateScoreCard = () => {
  ...
  const handleSubmit = () => {
    ...
    // Append profile picture
    formData.append("profile_picture", profile[0]); // Assuming you only need one profile picture for the contest
    ...
    axios
      .post("https://expoproject.saeedantechpvt.com/api/admin/judges", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${token}`,
        },
