    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '90vh', padding: '1rem 5%' }}>
        <Grid container spacing={4} sx={{ alignItems: 'start' }}>
          <Grid item xs={12} sm={3} md={2.5}>
            <Box>
              <ScoreBoard judgeName={judgeName} totalScore={totalScore} participantId="62" participantsName={participantsName} />
            </Box>
          </Grid>
          <Grid item xs={12} sm={9} md={9.5}>
            <Grid container spacing={2}>
              {judges.map((judge, index) => {
                const scores = currentParticipantScores.filter(score => score.judge_id === judge.id);
                return (
                  <Grid item xs={12} sm={6} md={3} key={index}>
                    <ParticipantCard
                      judge={judge}
